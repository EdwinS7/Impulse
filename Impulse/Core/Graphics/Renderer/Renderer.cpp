...
DrawCommand* CRenderer::WriteToBuffer( D3D_PRIMITIVE_TOPOLOGY primitive_topology, std::vector<Vertex>* vertices, std::vector<int32_t>* indices, Texture* texture, int z_index ) {
    ...
    auto Command = std::make_unique<DrawCommand>(
        primitive_topology, *vertices, *indices, texture, z_index
    );
