...
void CMemory::SetTarget( const char* target_name ) {
	m_Target = GetModuleHandleA( target_name );
}

template<typename T>
inline void CMemory::Write( std::uintptr_t address, T* value ) {
	WriteProcessMemory( m_Target, reinterpret_cast< LPVOID >( address ), &value, sizeof( T ), nullptr );
}

template<typename T>
T CMemory::Read( std::uintptr_t address ) {
	T Value{};

	ReadProcessMemory( m_Target, reinterpret_cast< LPCVOID >( address ), &Value, sizeof( T ), nullptr );
...
std::uintptr_t CMemory::ScanPattern( const std::string& signature ) {
    auto image = reinterpret_cast< std::uintptr_t >( m_Target );
